...
//  double distTrd, double angleTrd);
...
//  const std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> clusterRef);
    ...
    // local cluster frags to groups that all represent the same line segment
    std::vector<std::list<int> > localCluster(const std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> lines, double distTrd, double angleTrd) {
