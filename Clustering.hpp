...
#ifndef CCPERCEPTION2018_NEWPERCEPTION_CLUSTERING_HPP
#define CCPERCEPTION2018_NEWPERCEPTION_CLUSTERING_HPP
...
class CCluster {
 public:
    ...
    double pt2frag(std::pair<Eigen::Vector3f, Eigen::Vector3f> frag,
                   Eigen::Vector3f c) {
        Eigen::Vector3f a = frag.first;
        Eigen::Vector3f b = frag.second;
        Eigen::Vector3f ac = c - a;
        Eigen::Vector3f ab = b - a;
        Eigen::Vector3f Iab = ab / ab.norm();
        Eigen::Vector3f ad = ac.dot(Iab)*Iab;
        Eigen::Vector3f d = a + ad;
        Eigen::Vector3f da = -ad;
        Eigen::Vector3f db = b - d;
        if (da.dot(db) < 0) {
    ...
    bool ptOnFrag(std::pair<Eigen::Vector3f, Eigen::Vector3f> frag,
                  Eigen::Vector3f c) {
        Eigen::Vector3f a = frag.first;
        Eigen::Vector3f b = frag.second;
        Eigen::Vector3f ab = b - a;
        Eigen::Vector3f ac = c - a;
        Eigen::Vector3f Iab = ab / ab.norm();    // identity vector of ab
        Eigen::Vector3f ad = ac.dot(Iab) * Iab;
        Eigen::Vector3f d = a + ad;
        Eigen::Vector3f da = -ad;
        Eigen::Vector3f db = b - d;
    ...
    bool line2line(const std::pair<Eigen::Vector3f, Eigen::Vector3f> lines1,
                   const std::pair<Eigen::Vector3f, Eigen::Vector3f> lines2,
    ...
    bool frag2frag(const std::pair<Eigen::Vector3f, Eigen::Vector3f> ab,
                   const std::pair<Eigen::Vector3f, Eigen::Vector3f> cd) {
        Eigen::Vector3f a = ab.first;
        Eigen::Vector3f b = ab.second;
        Eigen::Vector3f c = cd.first;
        Eigen::Vector3f d = cd.second;
    ...
    double pt2G(const Eigen::Vector3f P, const Eigen::Vector3f dir,
    ...
    Eigen::Vector3f ptProj2Line(const Eigen::Vector3f P, const Eigen::Vector3f dir,
                                const Eigen::Vector3f a) {
        Eigen::Vector3f pa = a - P;
        Eigen::Vector3f pb = pa.dot(dir)*dir;
        Eigen::Vector3f b = P + pb;
        return b;
    }

    // use a frag to represent all frags that represent the same lien segment
    // get from local cluster
    std::vector<std::pair<Eigen::Vector3f, Eigen::Vector3f>> mergeFrag(
        const std::vector<std::pair<Eigen::Vector3f, Eigen::Vector3f>> lines,
        const std::vector<std::list<int> > clusters) {
        std::vector<std::pair<Eigen::Vector3f, Eigen::Vector3f>> clusterRef;
            ...
            iter != clusters.end(); iter++) {
                ...
                for (std::list<int>::iterator iter2 = linesIDs.begin();
                    iter2!= linesIDs.end(); iter2++, i+=2) {
                        Eigen::Vector3f a = lines[*iter2].first;
                        Eigen::Vector3f b = lines[*iter2].second;
                        P += a;
                        P += b;
                        direction += b - a;
                        points(0, i) = a[0];
                ...
                //        Eigen::Vector3f dir = Eigen::Vector3f(U(0,maxSValuePos),
                ...
                dists.reserve(n);
                for (std::vector<Eigen::Vector3f>::iterator iter2 = pts.begin();
                    iter2 != pts.end(); iter2++) {
                ...
                Eigen::Vector3f a = ptProj2Line(P, dir, pts[minIdx]);
                Eigen::Vector3f b = ptProj2Line(P, dir, pts[maxIdx]);

                clusterRef.push_back(std::pair<Eigen::Vector3f, Eigen::Vector3f>(a, b));
        }
    ...
    std::vector<std::list<int> > segCluster(std::vector<std::list<int> > clusters,
                                            const std::vector<std::pair<Eigen::Vector3f, Eigen::Vector3f>> clusterRef) {
        ...
        for (int i = 0; i < numCluster; i++) {
            std::pair<Eigen::Vector3f, Eigen::Vector3f> ab = clusterRef[i];

            for (int j = i+1; j < numCluster; ++j) {
                std::pair<Eigen::Vector3f, Eigen::Vector3f> cd = clusterRef[j];
    ...
    std::vector<std::list<int> > localCluster(const std::vector<std::pair<Eigen::Vector3f, Eigen::Vector3f>> lines, double distTrd, double angleTrd) {
        // get relation edge matrix A
        int num_lines = lines.size();
        std::vector<std::pair<int, int>> A;

        for (int i = 0; i < num_lines; ++i) {
            std::pair<Eigen::Vector3f, Eigen::Vector3f> ab = lines[i];

            for (int j = i+1; j < num_lines; ++j) {
                std::pair<Eigen::Vector3f, Eigen::Vector3f> cd = lines[j];
...
#endif  // CCPERCEPTION2018_NEWPERCEPTION_CLUSTERING_HPP
